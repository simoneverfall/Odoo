...
class ResCompany(models.Model):
    ...
    def create_imported_accounts(self, acc):
        ...
        account_acc = self.env['account.account'].search(
            ['|', ('xero_account_id', '=', acc.get('AccountID')), ('code', '=', acc.get('Code')),('company_ids', '=', self.id)])
        account_account = self.env['account.account'].search(
            [('id', 'in', account_acc.ids), ('company_ids', '=', self.id)])
    ...
    def create_imported_tax(self, acc):
        ...
        if acc.get('Status') == 'ACTIVE':
            if not account_tax:
                ...
                dict_t.update({'company_id': self.id})
                if not account_tax_name:
                    dict_t.update({'name': acc.get('Name')})
                    dict_t.update({'type_tax_use': 'sale'})
                    dict_t.update({'price_include': False, 'price_include_override': 'tax_excluded', })
                    ...
                    if account_tax_create_s:
                        ...
                        dict_t.update({'price_include': True, 'price_include_override': 'tax_included'})
    ...
    def create_imported_products(self, item):
        ...
        product_exists = self.env['product.product'].search(
            ['|', ('xero_product_id', '=', item.get('ItemID')), ('default_code', '=', item.get('Code')),('company_id', '=', self.id)])
        # product_exists = self.env['product.product'].search(
        ...
        if item.get('SalesDetails', False):
            ...
            if item.get('SalesDetails').get('AccountCode', False):
                acc_id_s = self.env['account.account'].search(
                    [('code', '=', item.get('SalesDetails').get('AccountCode')), ('company_ids', '=', self.id)])
                if acc_id_s:
        ...
        if item.get('PurchaseDetails', False):
            ...
            if item.get('IsTrackedAsInventory') == True:
                ...
                if item.get('PurchaseDetails').get('AccountCode', False):
                    acc_id_p = self.env['account.account'].search(
                        [('code', '=', item.get('PurchaseDetails').get('AccountCode')), ('company_ids', '=', self.id)])
                    if acc_id_p:
                        dict_create.update({'property_account_expense_id': acc_id_p.id})
                    else:
                        self.import_accounts()
        ...
        if item.get('IsTrackedAsInventory') and not product_exists:
            if item.get('IsTrackedAsInventory') == True:
                ...
                options = dict(product_master_model._fields['type'].inherited_field.selection)
                if 'consu' not in options:
                    raise ValidationError(
    ...
    def create_customer_for_invoice(self, cust, res_partner):
        ...
        if cust.get('Type') == 'ACCREC':
            sale = self.env['account.journal'].search([('type', '=', 'sale'),('company_id', '=', self.id)], limit=1)
            if sale:
                dict_i['journal_id'] = sale.id

        if cust.get('Type') == 'ACCPAY':
            purchase = self.env['account.journal'].search([('type', '=', 'purchase'),('company_id', '=', self.id)], limit=1)
            if purchase:
        ...
        if cust.get('LineItems'):
            ...
            if isinstance(order_lines, (dict,)):
                ...
                if not i.get('ItemCode'):
                    ...
                    res_product = self.env['product.product'].search([('default_code', '=', i.get('ItemCode')),('company_id','=',self.id)])
                    if res_product:
                        invoice_line_vals = self.create_invoice_line(i, res_product, cust, invoice_type, tax_state)
                    else:
    ...
    def create_invoice_line(self, i, res_product, cust, invoice_type, tax_state):
        ...
        if i.get('AccountCode'):
            acc_id_s = self.env['account.account'].search(
                [('code', '=', i.get('AccountCode')), ('company_ids', '=', self.id)])
            if acc_id_s:
    ...
    def create_customer_for_sale_order(self, cust, res_partner):
        ...
        if cust.get('CurrencyCode'):
            currency = self.env['res.currency'].sudo().search(
                ...
                limit=1)
            price_list = self.env['product.pricelist'].search([('currency_id', '=', currency.id),('company_id', '=', self.id)], limit=1)
            if not price_list:
        ...
        if cust.get('LineItems'):
            ...
            if isinstance(order_lines, (dict,)):
                ...
                res_product = self.env['product.product'].search(
                    [('default_code', '=', i.get('ItemCode')),('company_id','=',self.id)])
                if res_product:
    ...
    def create_imported_customers(self, item):
        if item.get('AccountNumber'):
            customer_exists = self.env['res.partner'].search(['|', ('active', '=', False),
                                                              ('active', '=', True),
                                                              ('xero_cust_id', '=', item.get('ContactID')),('company_id', '=', self.env.company.id)], limit=1)
            if not customer_exists:
                customer_exists = self.env['res.partner'].search(
                    [('name', '=', item.get('Name')), ('xero_cust_id', '=', False),('company_id', '=', self.env.company.id)], limit=1)

                if not customer_exists and item.get('EmailAddress'):
                    customer_exists = self.env['res.partner'].search(
                        [('email', '=', item.get('EmailAddress')), ('xero_cust_id', '=', False),('company_id', '=', self.env.company.id)], limit=1)
        else:

            customer_exists = self.env['res.partner'].search(
                ['|', ('active', '=', False),
                 ('active', '=', True), ('xero_cust_id', '=', item.get('ContactID')),('company_id', '=', self.env.company.id) ], limit=1)
            if not customer_exists:
                customer_exists = self.env['res.partner'].search(
                    [('name', '=', item.get('Name')), ('xero_cust_id', '=', False),('company_id', '=', self.env.company.id)], limit=1)
            ...
            if pay.get('Invoice').get('InvoiceID'):
                ...
                for i in invoice_pay:
                    ...
                    payment = self.env['account.payment'].search(
                        [('memo', '=', i.name), ('xero_payment_id', '=', False)], limit=1)
                ...
                if 'partner_id' in dict_g:
                    company = self
                    journal_id = self.env['account.journal'].get_journal_from_account(pay.get('Account').get('Code'),company)
...
class AccountJournal(models.Model):
    ...
    def get_journal_from_account(self, xero_account_code,company):
        ...
        account_id = self.env['account.account'].search(
            [('code', '=', xero_account_code), ('company_ids', '=', xero_config.id)])
